...
fn main() {
    ...
    for x in 0..max_steps + 2 {
        for y in 0..max_steps + 2 {
            let is_ok = a_star(start, (x,y))
                .map(|s| s.len() as isize - 1 <= max_steps)
                .unwrap_or(false);
            if is_ok {
                n += 1;
            }
...
fn a_star(start: (isize, isize), goal: (isize, isize)) -> Option<Vec<(isize,isize)>> {
